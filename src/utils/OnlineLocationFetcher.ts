 ...
 */
async function fetchFromNominatim(query: string, limit: number = 50): Promise<OnlineLocation[]> {
  try {
 ...
 */
function processLocation(location: any, source: 'nominatim' | 'overpass'): ProcessedLocation | null {
  try {
    let name = '';
    let coordinates: Coordinates;
    let type = '';
    let category: ProcessedLocation['category'] = 'landmark';
    let municipality = '';
    let barangay = '';
    let road = '';
    let postcode = '';
    ...
    if (source === 'nominatim') {
      ...
      if (!municipality) {
        ...
        municipality = addressParts.find((part: string) => 
          MUNICIPALITIES.some(mun => part.trim().toLowerCase().includes(mun.toLowerCase()))
        )?.trim() || '';
      }
    ...
    if (road && road !== name) addressParts.push(road);
    if (barangay && barangay !== name) addressParts.push(`Brgy. ${barangay}`);
    if (municipality && municipality !== name) addressParts.push(municipality);
 ...
 */
export async function fetchComprehensiveLocations(): Promise<ProcessedLocation[]> {
  ...
  const uniqueLocations = new Map<string, ProcessedLocation>();
 ...
 */
export async function generateComprehensiveDatabase(): Promise<void> {
  try {
    ...
    // Save to file
    const outputPath = path.join(__dirname, '..', 'data', 'siquijor-locations-comprehensive.json');
